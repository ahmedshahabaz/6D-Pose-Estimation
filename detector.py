...
class ObjectDetector(object):
	...
	def detect_object_in_image(net_model, pnp_solver, in_img, config):
		...
		image_torch = Variable(image_tensor).unsqueeze(0) #.cuda().unsqueeze(0)
		#image_torch = Variable(image_tensor).cuda().unsqueeze(0)
		out, seg = net_model(image_torch)
		vertex2 = out[-1][0]
		aff = seg[-1][0]

		print(np.shape(vertex2))
		print(np.shape(aff))
	...
	def find_object_poses(vertex2, aff, pnp_solver, config):
		...
		for obj in objects:
			...
			if(len(points) - points.count(None) < 6):
				print("###breaking out###",points)
				continue
			print("@Total Points: ",len(points),"Null Points: " ,points.count(None))
			location, quaternion, projected_points, rotation = pnp_solver.solve_pnp(points)
