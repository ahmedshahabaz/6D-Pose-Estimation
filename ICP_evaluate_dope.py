...
def ICP(p_gt,p_real):
	#print("------------ICP-------------")
	T, R_icp, t_icp = icp(p_gt.transpose(),p_real.transpose())
	#print(R_icp,"\n_____________\n",t_icp)
	#print("----------------------------------------------------------------")
	#print(R_real,"\n_____________\n",t_real)

	p_icp = np.zeros(p_real.shape)

	for i in range(x.shape[1]):
		#p_gt[:,i] = np.matmul(R_gt,x[:,i]) + t_gt
		p_icp[:,i] = np.matmul(R_icp,p_real[:,i]) + t_icp
...
if __name__ == "__main__":
	...
	t_real, q_real = test_image('000022.png',models,pnp_solvers,pub_dimension,draw_colors)
	...
	with open('./test/000054/scene_gt.json') as f:
		gt = json.load(f)
	f.close()
	for x in gt['22']:
		if x['obj_id']==3:
